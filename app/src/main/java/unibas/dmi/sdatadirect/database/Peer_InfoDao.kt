...
interface Peer_InfoDao {
    ...
    @Query("SELECT EXISTS(SELECT * FROM peer_info WHERE peer_key = (SELECT foreign_public_key FROM peer_table WHERE wifi_mac_address = :wifiAddress) AND feed_key = :feed_key AND isSubscribed = 'true')")
    fun isSubscribed(wifiAddress: String, feed_key: String): Boolean

    @Query("SELECT * FROM peer_info WHERE peer_key = (SELECT foreign_public_key FROM peer_table WHERE wifi_mac_address = :wifiAddress)")
