...
interface PeerDao {
    ...
    @Query("SELECT last_sync FROM peer_table WHERE foreign_public_key = :public_key")
    fun getLastSync(public_key: String): Long?

    @Query("UPDATE peer_table SET last_sync = :lastSync WHERE foreign_public_key = :public_key")
    fun setLastSync(public_key: String, lastSync: Long)

    @Query("SELECT peer_info.* FROM peer_table, peer_info WHERE foreign_public_key = :public_key AND peer_key = foreign_public_key")
    fun getPeerSubscriptions(public_key: String): Array<PeerInfo>?

    @Query("SELECT id FROM peer_table WHERE foreign_public_key = :public_key")
